...
def fetch_links():
    ...
    try:
        ...
        sb(f"Found {len(links)} unique links")
    except requests.RequestException as err:
        sb(f"Error: {str(err)}")
...
def fetch_title():
    ...
    try:
        ...
        sb(f'Page Title: {title}')  # Correctly display title in the status bar
...
if __name__ == "__main__": # execute logic if run directly
    ...
    _root.title('Clayton Dodd scraper')
    ...
    _fetch_btn_link = ttk.Button(
        _url_frame, text='Fetch link', command=fetch_links)  # Fetch links
    _fetch_btn_link.grid(row=2, column=1, sticky=W, padx=5)

    _fetch_btn_info = ttk.Button(
        _url_frame, text='Fetch img', command=fetch_url)  # Fetch images
    _fetch_btn_info.grid(row=0, column=1, sticky=W, padx=5)

    _fetch_btn_title = ttk.Button(
        _url_frame, text='Fetch title', command=fetch_title)  # Fetch title
    _fetch_btn_title.grid(row=1, column=1, sticky=W, padx=5)
